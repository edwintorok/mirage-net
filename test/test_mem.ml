...
let test_region_track _ () =
  ...
  let ( _ : Cstruct.t Lwt.t) =
    Cstruct.create packet
    |> Lwt.return
    |> track_lwt ~size_in_bytes:packet
  in
  ...
  let res = Cstruct.create packet |> sleep |> track_lwt ~size_in_bytes:packet in
...
let test_region_track_abandon _ () =
  ...
  let () = Cstruct.create packet |> sleep |> track_lwt ~size_in_bytes:packet |> ignore_lwt in
  Gc.full_major ();
  let used01 = region.bytes in
  Alcotest.V1.(check' int ~msg:"Region.get_bytes01" ~actual:used01 ~expected:used00);
  Lwt.return_unit
